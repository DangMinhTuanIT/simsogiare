...
<input type="hidden" name="" value="{{str_replace('/abc','',route('orders.edit',array('abc')))}}" class="page_edit">
...
@section('script')
    ...
    <script type="text/javascript">
        dataCenter.searchRoute = '{{ route('orders.search') }}';
        dataCenter.filterRoute = '{{ route('orders.list') }}';
        dataCenter.truncateRoute = '{{ route('orders.truncate') }}';
        var page_edit = jQuery('.page_edit').val();
        dataCenter.columns = [
            null,
            {data: 'name_customer', name: 'name_customer'},
            {data: 'phone_customer', name: 'phone_customer'},
            {data: 'phone_sim', name: 'phone_sim'},
        ...
        $(function () {
            ...
            var params = {
                route: '{{ route('orders.data') }}' + '?@php echo \App\myHelper::getFilters($filters); @endphp',
