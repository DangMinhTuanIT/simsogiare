...
@section('content')
        ...
        <a class="btn bg-teal btn-lg waves-effect m-b-15" href="{{route('category.add')}}">
            THÊM MỚI
        </a>
<input type="hidden" name="" value="{{URL::to('admin/category/edit/')}}" class="page_edit">
...
@section('script')
    ...
    <script type="text/javascript">
        dataCenter.searchRoute = '{{ route('category.search') }}';
        dataCenter.filterRoute = '{{ route('category.list') }}';
        dataCenter.truncateRoute = '{{ route('category.truncate') }}';
        ...
        dataCenter.tableConfigs.columnDefs = [
            {
            ...
            },
             {
                ...
                className: '',
                render: function (data, type, row){
                    return '<a target="_blank" href="'+home_url+'/category/'+ row['slug'] +'-ca'+ row['id'] +'.html">'+row['name']+'</a>';
                }
            },
          
            {
                ...
                render: function (data, type, row){
                    if(row['image_link']!=''){
                         return '<img src="'+home_url+'/'+ row['image_link'] +'" style="width:100px">';
                    }else{
                        return '<img src="https://atasouthport.com/wp-content/uploads/2017/04/default-image.jpg"  style="width:100px">';
                    }
                }
            },
        ...
        $(function () {
            ...
            var params = {
                route: '{{ route('category.data') }}' + '?@php echo \App\myHelper::getFilters($filters); @endphp',
