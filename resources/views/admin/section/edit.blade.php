...
@endsection
@section('content')
<form class="form_validation collapse{{ (old('data')) ? ' in' : '' }} in" enctype="multipart/form-data"  id="formDemo" action="{{route('section.update_db',array($id_section,$info->id))}}" method="post" role="form">
            ...
            <div class="panel panel-default">
                ...
                <div class="panel-body">
                    <div class="form-group">
                        ...
                        <input type="text" id="name_section" name="name_section" value="{{$info->name_section}}" class="form-control" placeholder="Tên hiển thị đầu số, ví dụ: Đầu số 097">                    </div>
                        <div class="form-group">
                        <label><label for="number_section">Đầu số</label></label>
                        <input type="text" id="number_section" name="number_section" class="form-control" value="{{$info->number_section}}" placeholder="Đầu số của nhà mạng, ví dụ: 097">                    </div>
                     <div class="form-group">
                        <label><label for="parent_id">Chọn thể loại Cha</label></label>
                        <select id="parent_id" name="parent_id" class="form-control">
                            <option value="0" >== Không có ==</option>
                            @foreach($parrent as  $item)
                             @php $active = $info->parent_id==$item->id ? 'selected="selected"' : '' @endphp
                    ...
                    <div class="form-group">
                      ...
                      <p class="help-block">Vị trí sắp xếp của đầu số, số nhỏ đứng trước.
</p>
                    </div>
