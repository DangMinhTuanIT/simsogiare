...
@section('content')
<a class="btn bg-teal btn-lg waves-effect m-b-15" href="{{route('section.add',array($id_section))}}">
            THÊM MỚI
        </a>
        
<input type="hidden" name="" value="{{str_replace('/abc','',route('section.edit',array($id_section,'abc')))}}" class="page_edit">
...
@section('script')
    ...
    <script type="text/javascript">
        dataCenter.searchRoute = '{{ route('section.search',array($id_section)) }}';
        dataCenter.filterRoute = '{{ route('section.list',array($id_section)) }}';
        dataCenter.truncateRoute = '{{ route('section.truncate') }}';
        ...
        $(function () {
            ...
            var params = {
                route: '{{ route('section.data',array($id_section)) }}' + '?@php echo \App\myHelper::getFilters($filters); @endphp',
