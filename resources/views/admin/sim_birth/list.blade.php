...
@section('content')
<a class="btn bg-teal btn-lg waves-effect m-b-15" href="{{route('sim_birth.add')}}">
            THÊM MỚI
        </a>
        
<input type="hidden" name="" value="{{str_replace('/abc','',route('sim_birth.edit',array('abc')))}}" class="page_edit">
...
@section('script')
    ...
    <script type="text/javascript">
        dataCenter.searchRoute = '{{ route('sim_birth.search') }}';
        dataCenter.filterRoute = '{{ route('sim_birth.list') }}';
        dataCenter.truncateRoute = '{{ route('sim_birth.truncate') }}';
        var page_edit = jQuery('.page_edit').val();
        dataCenter.columns = [
            null,
            {data: 'id', name: 'id'},
            {data: 'name_birth', name: 'name_birth'},
        ...
        $(function () {
            ...
            var params = {
                route: '{{ route('sim_birth.data') }}' + '?@php echo \App\myHelper::getFilters($filters); @endphp',
