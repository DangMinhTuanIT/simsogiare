...
@section('content')
<a class="btn bg-teal btn-lg waves-effect m-b-15" href="{{route('price_range.add')}}">
            THÊM MỚI
        </a>
        
<input type="hidden" name="" value="{{str_replace('/abc','',route('price_range.edit',array('abc')))}}" class="page_edit">
...
@section('script')
    ...
    <script type="text/javascript">
        dataCenter.searchRoute = '{{ route('price_range.search') }}';
        dataCenter.filterRoute = '{{ route('price_range.list') }}';
        dataCenter.truncateRoute = '{{ route('price_range.truncate') }}';
        var page_edit = jQuery('.page_edit').val();
        dataCenter.columns = [
            null,
            {data: 'weight', name: 'weight'},
            {data: 'name_price_range', name: 'name_price_range'},
            {data: 'price_start_int', name: 'price_start_int'},
            {data: 'price_end_int', name: 'price_end_int'},
        ...
        $(function () {
            ...
            var params = {
                route: '{{ route('price_range.data') }}' + '?@php echo \App\myHelper::getFilters($filters); @endphp',
