...
class SimFate extends Model{
	protected $table = 'sim_fate';
    ...
    {
        return DB::table('sim_fate as s')->select('s.*')->get();
    }

    public static function search($params = ''){
        $filters = [];
        $items = DB::table('sim_fate as s')
        ->where('s.name_sim_fate', 'like', '%' . $params . '%')
        ->orderBy('s.weight','asc')
            ->select('s.*');
    ...
    public static function removeAll(){
        return DB::table('sim_fate')->truncate();
    }

    public static function batchRemove($params){
        return DB::table('sim_fate')
            ->whereIn('id', $params)
            ->delete();
    }
