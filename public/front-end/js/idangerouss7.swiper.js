...
var Swiper = function (selector, params) {
    ...
    if ((_this.browser.ie10 || _this.browser.ie11) && !params.onlyExternal) {
        _this.wrapper.classList.add('swiper-wp8-' + (isH ? 'vertical' : 'vertical'));
    }
    ...
    setWrapperTranslate : function (x, y, z) {
        ...
        // passed one coordinate and optional axis
            ...
            if (typeof y === 'undefined') {
                y = this.params.mode === 'vertical' ? 'x' : 'y';
            }
            coords[y] = x;
        }
        sessionStorage.setItem("scroll", coords.x);
        var height_card__content = jQuery('.active .swiper-wrapper').outerHeight() - 
        (jQuery('.encounters__card.active').height());
        var tinh = 50/height_card__content;
        var top_n = -((parseInt(sessionStorage.getItem("scroll"))*tinh));
        jQuery('.active .profile-card-scroll-bar-value').css({"top" : ""+top_n+"px","transition-duration":"1s"},100);
        if( coords.x < -height_card__content ){
            jQuery('.encircle .arrow').addClass('active');
           jQuery('.encircle .arrow').fadeIn();
          
         }
        
        if (this.support.transforms && this.params.useCSS3Transforms) {
            translate = this.support.transforms3d ? 'translate3d(' + coords.y + 'px, ' + coords.x + 'px, ' + coords.z + 'px)' : 'translate(' + coords.x + 'px, ' + coords.y + 'px)';
